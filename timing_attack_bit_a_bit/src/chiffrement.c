...
{ 
   ...
   */
   ...
   //###########################-TIMING ATTACK-#################################//
      ...
      if(TIMING_ATTACK_CONFIRMED && DECRYPT && target_bit == position_bit)
   ...
   //###########################-TIMING ATTACK-#################################//
      if(TIMING_ATTACK_CONFIRMED && DECRYPT && target_bit == position_bit)
      {
         ...
         calculer_temps_moyen(&A, target_bit);  //calcul Ta
         calculer_temps_moyen(&B, target_bit);  //calcul Tb

         //met le résultat de la difference dans la variable globale T
         calculer_difference_temps_moyen(&A, &B, target_bit);
      }
   //###########################################################################//
   ...
   for(k = taille; k > 0; k--)
   {
      ...
      {  
         //###########################-TIMING ATTACK-#################################//
            ...
            //on commence l'attaque à dk-2
            if(k <= taille - 1 && DECRYPT)   //si dk-i ≤ dk-2
               position_bit = k - 1;   // position = dk-i
         //###########################################################################//
